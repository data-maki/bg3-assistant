...
Builds and items live in relational tables inside the shared SQLite database (`backend/app/catalog.py`). The checked-in TSV exports seed the catalog exactly once per guide version (`catalog_seed_version` setting); after that the database is authoritative and the Google Sheet's per-build tabs (everything after "Inspiration on the go") are no longer a data source. A URL import inserts one `builds` row plus `build_items` joins, upserting unknown items with a best-effort bg3.wiki effect lookup — reviewed catalog facts always win over an import's sparse copy. Legacy `custom_loadouts` JSON blobs migrate into these tables on first launch and the blob table is dropped. `GET /api/items` serves the catalog to the native slot picker.

Act 1 has the reviewed route and local map. Act 2 currently has sourced equipment coordinates and a public Shadow-Cursed Lands map handoff. Its route is intentionally marked unavailable, which keeps the Act 2 to Act 3 gate locked.

The native app starts and owns the packaged backend. The browser map and native overlay share run state and validated imported builds through the same SQLite database. A build import never changes the party; assignment remains native player-owned state. Import fetching rejects non-public destinations and sends only extracted public-page text to OpenRouter's `google/gemini-3-flash-preview` structured-output endpoint.
...
- `Party`: whole roster, active-four selection, levels, builds, respec, and Withers hirelings.
- `Loadout`: active-character carousel, abilities, URL import, and current-act equipment.
