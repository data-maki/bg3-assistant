...
|---|---|---:|---|---|---|---|---|
| BUILD-RID-01 | arm64, x64 | Critical | Before the fix, `Directory.Build.props` returned `PlatformTarget=x64`, `Platforms=x64`, while App, GameWindowHost, and OverlayHarness each declared `RuntimeIdentifier=win-x64`. Supplying an ARM64 intent could not change the graph consistently. | Architecture was an implicit shared x64 default plus repeated executable-project overrides. | Require `-p:BG3Architecture=arm64|x64`; derive and validate RID, platform, and platform target centrally; reject missing, x86, RID mismatch, platform mismatch, and changed supported-RID lists. Direct MSBuild negative cases now fail with the expected actionable errors. | Review `a1fab44` invalidated caller-controlled helper checks and one-way SLNX mapping; literal canonical checks and symmetric Platform selection added, pending re-review. | Visual Studio UI switching is not locally exercised; solution entrypoints are automated. |
| BUILD-RID-02 | arm64 | High | Lock audit showed only App and two spikes had `win-x64` targets; all other locks were architecture-neutral. A locked ARM64 restore therefore had no complete per-project ARM64 graph. | Locks reflected the x64-only/neutral project declarations rather than the two supported runtime graphs. | Declare exactly `win-x64;win-arm64` centrally and regenerate all 11 project lock files. Locked solution restores pass for both explicit inputs, and every lock now contains both RID targets. | Review `a1fab44` confirmed both locked graphs and all 11 lock inventories; canonical list check correction pending re-review. | NuGet server availability and future package updates can change asset graphs; CI retains locked mode. |
| BUILD-RID-03 | arm64 | Critical | `tools/build-release.ps1` defaulted its path to `win-x64`, passed `--runtime win-x64`, and used a user-local SDK path. Invoking it for ARM64 was impossible; invoking from the repository root also bypassed `windows/global.json`. | The release contract encoded one developer machine and one RID instead of accepting architecture. | Make `-Architecture arm64|x64` mandatory, derive `win-$Architecture`, resolve `dotnet` from `PATH`, run from the Windows root so `global.json` is honored, and isolate outputs under `artifacts/publish/<RID>`. Both publishes passed PE/deps inspection. | Review `a1fab44` confirmed script cwd/default isolation; CI SDK installation pin correction pending re-review. | Signing/MSIX creation belongs to the packaging lane and was not exercised here. |
| BUILD-RID-04 | arm64, x64 | Critical | CI restored and built only the app on `windows-latest`; it ran no tests and did not publish, package, inspect, or retain architecture evidence. It could not detect test-host, solution, or native-asset contamination. | CI had no architecture matrix and no full-solution or artifact validation. The solution itself exposed only inferred AnyCPU configuration/mappings. | Add explicit x64 (`windows-latest`) and ARM64 (`windows-11-arm`) jobs, verify runner ISA, locked-restore/build/test all projects, self-contained publish, package, PE/deps/MSIX inspection, and artifact upload. Add exact x64/arm64 SLNX configurations/mappings and a solution-level architecture projection. | Review `a1fab44` invalidated PE coverage, process-ISA proof, zero-test handling, package interface assumptions, and SDK pinning; all are corrected in-lane, pending integrated CI and re-review. | The workflow depends on the packaging lane's matching `build-development-msix.ps1 -Architecture` contract. GitHub CI has not run from this branch. Neither CI VM is physical native-x64 hardware proof. |
...
| Self-contained ARM64 publish inspection | Pass | All 487 PE files classified: 167 target ARM64, 320 verified neutral managed IL; exact deps target/native inventory and ARM64 SQLite present. Inspector itself ran ARM64. |
| Self-contained x64 publish inspection | Pass | All 488 PE files classified: 168 target x64, 320 verified neutral managed IL; exact deps target/native inventory and x64 SQLite present. |
...
| Reviewer arbitrary-name PE challenge | Expected failure | Adding ARM64 `unlisted-native.dll` to a copied x64 publish rejects machine `0xAA64`; no fixed filename allowlist remains. |
| Reviewer malformed-PE challenge | Expected failure | Adding a non-PE `malformed.dll` to a copied x64 publish fails closed. |
| Reviewer helper override challenges | Expected failure | Overriding the old expected-RID helper plus RID, or supported-list helper plus list, cannot change literal canonical checks. |
| SLNX Platform-only selection | Pass | `Platform=arm64` and `Platform=x64` both validate without hidden inputs; conflicting `Platform=arm64`/`BG3Architecture=x64` fails. |
| Explicit test project and shared testhost assertion inventory | Pass | All five projects explicitly set `IsTestProject=true` and include one shared architecture assertion. |
| x64 shared testhost assertion under emulation | Pass | 1/1; testhost reported x64 with 8-byte pointers. This remains emulated x64, not physical native-x64 proof. |
...
### Independent review correction (`a1fab44`)

- BUILD-RV-001: fixed by recursively scanning every file, failing closed for malformed `.exe`/`.dll` or malformed `MZ` images, parsing PE/CLI headers, allowing only target-machine PEs or provably IL-only neutral managed assemblies, rejecting 32-bit flags, validating exact deps target/native/runtime assets, requiring target SQLite, and applying the same scan after MSIX extraction.
- BUILD-RV-002: fixed by removing trusted expected/supported helper properties from validation and comparing caller inputs directly with canonical literals. Both reviewer override combinations now fail.
- BUILD-RV-003: CI now checks OS ISA, PowerShell ISA, exact SDK, parsed .NET host ISA, and a shared assertion executed inside every testhost.
- BUILD-RV-004: every test project explicitly identifies itself; CI requires a nonempty project set, one expected TRX per project, and a positive executed counter.
- BUILD-RV-005: packaging files remain owned by the packaging lane. CI now inspects the integrated script interface and requires mandatory `-Architecture` with exactly `arm64,x64` plus `-Version` before invocation.
- BUILD-RV-006: solution `Platform=arm64|x64` now becomes the explicit architecture when `BG3Architecture` is absent, while either direction and all conflicts remain validated.
- BUILD-RV-007: setup installs exactly SDK `10.0.302` using `windows/global.json`; CI also rejects any selected version other than `10.0.302`.
- Reviewer status after corrections: **pending independent rerun and integrated CI**. No correction is represented as accepted merely because this worker reproduced the challenges.
