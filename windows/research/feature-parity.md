...
“Windows x86” means **Windows 11 x86-64/AMD64**. The planned runtime identifier and package architecture are `win-x64` and `x64`. BG3 itself requires a 64-bit processor and OS, so nothing in the stated product requires 32-bit x86 ([BG3 system requirements](https://store.steampowered.com/app/1086940/Baldurs_Gate_3/)). A later 32-bit requirement would need a separate `win-x86` publish, MSIX, native SQLite binary, interop validation, and test matrix; it is not a compatible rename of the x64 artifact.
...
## Chat, OpenRouter, screen capture, and speech
...
| A-03 | macOS currently names `google/gemini-3-flash-preview`. | Initial Windows default: `google/gemini-3.6-flash`, verified through OpenRouter’s public model catalog on 2026-07-25 to accept text/image inputs and structured outputs. Keep it in one release configuration constant and validate capabilities in CI before every release; do not silently switch models at runtime ([model page](https://openrouter.ai/google/gemini-3.6-flash)). | Adaptation: current model |
| A-04 | Chat grounds answers with current step/route/party context, recent history, bundled source metadata, quick prompts, and scope selection. | Preserve prompt construction, current/route/party scopes, quick prompts, source display, Markdown, links, and image rendering. | Equivalent |
...
| A-07 | Opening chat may automatically prepare a one-window screenshot; user can preview, retake, remove, and send it only with the next prompt (`AppState+Capture.swift`). | Do not auto-capture. “Attach BG3 screenshot” launches the secure `GraphicsCapturePicker`; the player chooses BG3, Windows shows its capture border, one frame is kept in memory, and the session closes. Preserve preview/retake/remove/one-message lifetime. Windows recommends the picker and visible border for consent ([screen capture](https://learn.microsoft.com/en-us/windows/apps/develop/media-authoring-processing/screen-capture)). | Adaptation: explicit consent |
| A-08 | Screenshot is JPEG, bounded to the BG3 window, and the overlay is not included. | Capture only the selected BG3 HWND/item, convert HDR to SDR where necessary, encode JPEG in memory, and never write it to disk. A separate overlay HWND is not part of the selected game window. | Equivalent |
| A-09 | Speech button requests speech/microphone permission on use, supplies partial transcript, and writes the final transcript into chat draft (`SpeechInputService.swift`). | Packaged `Windows.Media.SpeechRecognition.SpeechRecognizer` with `HypothesisGenerated` and continuous result events; request microphone access only when the player presses the mic. The API requires MSIX identity and the microphone capability; dictation also depends on Windows Online speech recognition ([speech recognition](https://learn.microsoft.com/en-us/windows/apps/develop/input/speech-recognition), [packaged identity requirement](https://learn.microsoft.com/en-us/windows/apps/develop/input/manage-issues-with-audio-input)). | Equivalent |
...
| S-10 | Diagnostics expose BG3, guide, provider/AI, and error state. | Preserve plus Windows package version, OS build, process/window match, capture/speech availability, and signing channel. Never include the API key or captured image. | Equivalent |
...
| I-10 | Swift tests cover run safety, onboarding, run creation, act locks, manual builds/assets, imports, and gear; legacy backend endpoint tests also exist. | Port domain tests to xUnit first; omit only the unreachable-backend tests. Add Windows platform contract tests for credentials, SQLite, manifest, package contents, detection, positioning, capture, speech, and startup. | Equivalent plus required Windows coverage |
| I-11 | macOS validation builds/tests Swift, validates a no-signing app, and also runs Python backend tests on Linux (`scripts/macos/validate.sh`, `.github/workflows/ci.yml`). | Add a Windows x64 CI job: restore locked NuGets, build, xUnit, package, inspect manifest/content, verify no forbidden runtimes/services/secrets, and signature-verify release candidates. Existing macOS CI remains unchanged. | Equivalent |
| I-12 | macOS release scripts build/sign/notarize ZIP/TestFlight artifacts and currently have a source-build install path. | Produce one self-contained x64 MSIX and `.appinstaller` feed; prefer Microsoft Store for stable distribution and a signed HTTPS App Installer feed for beta. Windows App Installer is built in, and MSIX provides package identity and clean servicing ([App Installer](https://learn.microsoft.com/en-us/windows/msix/app-installer/app-installer-root), [MSIX overview](https://learn.microsoft.com/en-us/windows/msix/overview)). | Adaptation |
| I-13 | macOS permissions include screen recording, microphone/speech, login item, keychain, and outbound HTTPS. | Windows uses picker-mediated graphics capture, microphone capability/privacy settings, Online speech toggle for dictation, startup task, Credential Manager, and outbound HTTPS. No admin, accessibility/UIAccess, driver, inbound listener, private-network, game-file, process-injection, or save access. | Adaptation |
...
Windows feature parity is complete only when every row marked Equivalent or Adaptation has an automated or manual acceptance result, every Exclusion is still intentional, and the Act 2 Data gap is still visibly explained. The implementation must not count “code exists” as parity: it must pass the real BG3 DirectX 11/Vulkan, multi-monitor/DPI, capture, speech, install/update, and clean-machine tests in `implementation-plan.md`.
